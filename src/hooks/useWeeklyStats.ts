...
export const useWeeklyStats = () => {
  ...
  const fetchWeeklyStats = async () => {
    try {
      ...
      let statsData: StatsRecord[] = [];
      try {
        statsData = await pb.collection('stats').getFullList<StatsRecord>({ sort: 'date' });
      } catch {
        // No stats data
      }
      ...
      if (statsData.length > 0) {
        ...
        const userResponses = await pb.collection('user_responses').getFullList<UserResponse>({ sort: 'created' });
